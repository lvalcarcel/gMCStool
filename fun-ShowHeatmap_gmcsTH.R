ShowHeatmap_gmcsTH <- function(gene.exp, gene.ratio, gMCS.info.all, gMCS.info.target, sample.class, 
                               sample.class.target, sample.cohort, flag_include_boxplots_heatmap, 
                               flag_show_SYMBOL, colors_heatmap, colors_annotation, TABLE_GMCS_MODE){ 
  ...
  gene.set <- log2(as.matrix(gene.ratio[gmcs.ENSEMBL,]))
  ...
  if (length(gmcs.ENSEMBL)==1)  {
    gene.set <- matrix(log2(as.matrix(gene.ratio[gmcs.ENSEMBL,])), nrow = 1)
  ...
  if (flag_include_boxplots_heatmap){
    ...
    for (ch in levels(sample.cohort)) {
      ...
      plot.list.2[[ch]] <- ggboxplot(sdf, x = "Var1", y  ="exp", fill = "class") + theme_bw() + 
        ...
        geom_hline(yintercept = 1, col = "red") + guides(fill = guide_legend(nrow = 1)) + 
      ...
      if (LogC) { plot.list.2[[ch]] <- plot.list.2[[ch]] + scale_y_continuous(trans = "log1p", limits = c(0,NA))}
