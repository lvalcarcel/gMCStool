...
CalculateEssentialGenes_gmcsTH <- function(gene.exp, gMCS.info, sample.class, gmcsTH_perc, 
                                           nWorkers, threshold_logFC = 1e-2){
  ...
  # Calculate first second and third most expressed gene in each gMCS in each sample ####
  ...
  index_gmcs_length_1 <- as.numeric(which(apply(gMCSs.ENSEMBL.mat,1,sum)==1))
  ...
  # Calculate first second and third most expressed gene ratio in each gMCSs in each sample ####
  ...
  withProgress(message = 'Calculating most expressed gene by gMCS and sample',
               detail = '\nCalculating ratio...', {
                 ...
                 {
                   incProgress(1/dim(gene.first.exp)[2])
                   if (i%%10==0){print(paste("UNIT",':',i, dim(gene.first.exp)[2]))}
                   ...
                   ratio_threshold[i] <- quantile(aux[aux>0], gmcsTH_perc)
                 }
                 # })
               })
  ...
  if (dim(gene.first.exp)[1]>0){
    ...
    {
      if (i%%50==0){print(paste("UNIT", gmcsTH_perc, i, dim(gene.first.ratio)[2]))}
  ...
  obtain_essential_single_genes_gMCS <- function(i){
    ...
    # aux1 <- gene.first.ENSEMBL==gen 
    # aux1 <- aux1 & essentail.single.gMCS
    # aux2 <- colSums(aux1)
  ...
  #   
  #   return(list(num.essential.gene = num.essential.gene, 
