ShowHeatmap_localT2 <- function(gene.exp, localT2, gMCS.info.all, gMCS.info.target, 
                                sample.class, sample.class.target, sample.cohort, 
                                flag_include_boxplots_heatmap, flag_show_SYMBOL, colors_heatmap, colors_annotation, TABLE_GMCS_MODE){
  
  colors_annotation <- list(rainbow, scales::hue_pal())[[as.numeric(as.character(colors_annotation))]]
  sample.class.target <- sample.class.target[1:length(levels(sample.class))]
  sample.class.target[is.na(sample.class.target)] <- F
  # print(paste("is target", paste(sample.class.target)))
  ...
  gene.log.ratio <- gene.exp[names(localT2[[1]]$MAYBEON),]*0
  for (k in 1:ncol(gene.log.ratio))
  {
  ...
  gene.set <- as.matrix(gene.log.ratio[gmcs.ENSEMBL,])
  gene.set.exp <- as.matrix(gene.exp[gmcs.ENSEMBL,])
  # colnames(gene.set) <- sample.class
  rownames(gene.set) <- gmcs.ENSEMBL
  # try({
  if (length(gmcs.ENSEMBL)==2) { 
    gene.set <- gene.set[c(gen.ENSEMBL, gmcs.ENSEMBL[gmcs.ENSEMBL!=gen.ENSEMBL]),]
  } else if (length(gmcs.ENSEMBL)>2) {
    gene.set2 <- gene.set[gmcs.ENSEMBL!=gen.ENSEMBL,]
  ...
  for (cl in levels(sample.class)[sample.class.target>0.5]) {
    ...
    gene.set.rest <- gene.set[-1,sample.class==cl]
  ...
  if (length(gmcs.ENSEMBL)==1){
    for (ch in levels(sample.cohort)) {
      gene.set.list[[ch]] <- matrix(gene.set.list[[ch]], nrow = 1)
      gene.set.exp.list[[ch]] <- matrix(gene.set.exp.list[[ch]], nrow = 1)
      colnames(gene.set.list[[ch]]) <- COLNAMES
      colnames(gene.set.exp.list[[ch]]) <- COLNAMES
  ...
  for (ch in levels(sample.cohort)) {
    ...
    if (length(gmcs.ENSEMBL)>1){
      ...
      gene.set.rest <- gene.set.list[[ch]][-1,]
      if (length(gmcs.ENSEMBL)==2)  {  gene.set.rest <- matrix(gene.set.rest, nrow = 1) }
    ...
    if (sum(sample.class.target) > 0.5) {
      aux <- levels(sample.class)[sample.class.target>0.5]
      aux <- intersect(aux, sample.class.list[[ch]])
      if (TABLE_GMCS_MODE=="percentage") {
        aux <- paste(aux, " ", round(gMCS.info.target[,aux]*100,1), "%", sep = "", collapse = ", ")
      } else {
    ...
    plot.list[[ch]] <- pheatmap(gene.set.list[[ch]],
                                ...
                                # color = circlize::colorRamp2(c(-1, 0, +1), c("royalblue1", "white", "firebrick1")),
  ...
  if (flag_include_boxplots_heatmap){
    ...
    for (ch in levels(sample.cohort)) {
      ...
      sdf$Var3 <- match(sdf$Var2,colnames(gene.set.exp.list[[ch]]))
      sdf$class <- sample.class.2.list[[ch]][sdf$Var3]
      sdf$exp <- sdf$value
      ...
      plot.list.2[[ch]] <- plot.list.2[[ch]] + theme(axis.text.y.right = element_text(color = "red"))
      
      if (LogC) {sdf$log2exp <- log2(sdf$exp+1)} else {sdf$log2exp <- sdf$exp}
      
      # Add p-values of `stat.test` and `stat.test2`
      if (min(table(sdf$class)/length(gmcs.ENSEMBL))>1){
        try({
          stat.test <- sdf %>%
            group_by(Var1) %>%
          ...
          stat.test <- stat.test %>% filter(grepl("responder", group1))  %>% filter(grepl("responder", group2))
          stat.test %>% filter(sub(".responder", "", sub(".non.responder", "", group1)) == sub(".responder", "", sub(".non.responder", "", group2)))
    ...
    pp_boxplot <-  ggpubr::ggarrange(plotlist = plot.list.2, nrow = 1, ncol = length(levels(sample.cohort)),
                                     widths = log10(unlist(levels.list.2))+2)
